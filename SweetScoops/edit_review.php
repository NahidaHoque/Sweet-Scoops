...
if (!isset($_GET["review_id"])) {
    echo "Review ID not provided in the URL.";
    exit();
}

// Get review_id from the URL
$review_id_from_url = mysqli_real_escape_string($conn, $_GET["review_id"]);

// Fetch review information based on the review_id
$sql = "SELECT * FROM review_details WHERE review_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $review_id_from_url);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    echo "No review found with the provided review ID.";
    exit();
}
...
if (isset($_POST['submit_review'])) {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("UPDATE review_details SET review_des = ?, username = ?, email = ? WHERE review_id = ?");
    $stmt->bind_param("sssi", $review, $username, $email, $review_id_from_url);

    // Execute the statement
    if ($stmt->execute()) {
        //echo "Review updated successfully.";
        header("Location: edit_review.php?review_id=" . $review_id_from_url);
    } else {
            ...
            <div class="profile-info col-md-9">
                <div class="panel">
                    <div class="panel-body bio-graph-info">
                        <h1>Edit Review</h1>
                        <form action="edit_review.php?review_id=<?= $review_id_from_url ?>" method="post" enctype="multipart/form-data">
                            <div class="row">
                                <div class="bio-row">
                                    <div class="form-group">
                                        <label for="name"><span>User Name</span>:</label>
                                        <input type="text" class="form-control" id="name" name="name" value="<?= htmlspecialchars($user_name) ?>" readonly>
                                    </div>
                                </div>
                                <div class="bio-row">
                                    <div class="form-group">
                                ...
                                <div class="bio-row">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="userid" name="userid" value="<?= htmlspecialchars($user_id) ?>" readonly>
                                    </div>
                                </div>
                ...
                <div class="panel" style = "margin-top:20px;">
                    <div class="panel-body bio-graph-info">
                <form action="edit_review.php?review_id=<?= $review_id_from_url ?>" method="post">
                            <div class="form-group">
                            ...
                            <button type="submit_review" name="submit_review" class="btn btn-primary">Edit Review</button>
                </form>
            </div>
        </div>
    </div>
