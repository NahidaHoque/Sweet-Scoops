...
$sql = "SELECT  username, profile_image_path FROM user WHERE user_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $user_id);
$stmt->execute();
$stmt->bind_result( $userName, $profileImagePath);
$stmt->fetch();
$stmt->close();

$sql1 = "SELECT order_id, user_id, username, email, address, product_name, product_price, contact, today_date, delivery_date FROM `order` WHERE user_id = ?";
...
// Fetch all records
$orders = array(); // Initialize an array to store orders
while ($stmt1->fetch()) {
    $order = array(
        'order_id' => $order_id,
        'user_id' => $user_id,
            ...
            <div class="profile-nav col-md-3">
                <div class="panel">
                    ...
                    <ul>
                        ...
                        <li class="active"><a href="show_oderdetails.php"> <i class="fas fa-shopping-basket"></i> My order </a></li>
                ...
                <div class="panel">
                    <div class="panel-body bio-graph-info">
                        ...
                        <div class="bio-row">
                            ...
                            <div class="bio-row">
                            <p><span>Order ID</span>: <?= htmlspecialchars($order['order_id']) ?></p>
                        </div>
                        <div class="bio-row">
                            <p><span>User ID</span>: <?= htmlspecialchars($order['user_id']) ?></p>
                        </div>
                        <div class="bio-row">
                            <p><span>User Name </span>: <?= htmlspecialchars($order['username']) ?></p>
                        </div>
                        <div class="bio-row">
                            <p><span>Product Name </span>: <?= htmlspecialchars($order['product_name']) ?></p>
                        </div>
                        <div class="bio-row">
                            <p><span>Product Price </span>: <?= htmlspecialchars($order['product_price']) ?></p>
                        </div>
                        <div class="bio-row">
                            <p><span>Address</span>: <?= htmlspecialchars($order['address']) ?></p>
                        </div>
                        <div class="bio-row">
                            <p><span>Email </span>: <?= htmlspecialchars($order['email']) ?></p>
                        </div>
                        <div class="bio-row">
                            <p><span>Contact </span>: <?= htmlspecialchars($order['contact']) ?></p>
                        </div>
                        <div class="bio-row">
                            <p><span>Order date </span>: <?= htmlspecialchars($order['today_date']) ?></p>
                        </div>
                        <div class="bio-row">
                            <p><span>Delivery Date </span>: <?= htmlspecialchars($order['delivery_date']) ?></p>
                        </div>
                            </div>
                            <?php endforeach; ?>
                        </div>
